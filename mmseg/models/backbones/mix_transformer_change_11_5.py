...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W, return_att=False):
        # print("H, W", H, W)
...
class Block(nn.Module):
    ...
    def forward(self, x, H, W, return_att=False):
        # If return_att is True, skip the remaining operations and return the attention map
        if return_att:
            return self.attn(self.norm1(x), H, W, return_att=return_att)
        else:
            y = self.attn(self.norm1(x), H, W)
        
        # If not returning attention, continue with the residual connection and subsequent layers
...
class MixVisionTransformer(BaseModule):
    ...
    def forward_features(self, x, return_att=False, attn_stage=None):
        B = x.shape[0]
        outs = []
        ...
        for i, blk in enumerate(self.block1):
            # x = blk(x, H, W) # TODO: return to this later

            # # NOTE: add attention stuffs here
            if return_att and attn_stage == 1:
                return blk(x, H, W, return_att=True)
            else:
                x = blk(x, H, W)
        ...
        for i, blk in enumerate(self.block2):
            ...
            # NOTE: add attention stuffs here
            if return_att and attn_stage == 2:
                return blk(x, H, W, return_att=True)
            else:
                x = blk(x, H, W)
        ...
        for i, blk in enumerate(self.block3):
            ...
            # NOTE: add attention stuffs here
            if return_att and attn_stage == 3:
                return blk(x, H, W, return_att=True)
            else:
                x = blk(x, H, W)
        ...
        for i, blk in enumerate(self.block4):
            ...
            # NOTE: add attention stuffs here
            if return_att and attn_stage == 4:
                return blk(x, H, W, return_att=True)
            else:
                x = blk(x, H, W)
