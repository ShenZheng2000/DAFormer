...
if __name__ == '__main__':
    ...
    attentions = model.forward_features(img.to(device), return_att=True)
    # print("attentions shape", attentions.shape) # [1, 1, 14400, 225]

    nh = attentions.shape[1] # number of head

    # print(f"Number of head: {nh}") # 8

    # NOTE: do not drop the last path
    # we keep only the output patch attention
    # attentions = attentions[0, :, 0, 1:].reshape(nh, -1)
    # attentions = attentions[0, :, 0, :].reshape(nh, -1)

    attentions = attentions[0, :, :, 0].reshape(nh, -1) # NOTE: anurag write in 11/4
    # print("attentions shape NEW", attentions.shape) # [1, 14400]
    ...
    # print("attentions.shape is", attentions.shape)
    # NOTE: hardcode reshape size here
    # attentions = attentions.reshape(nh, w_featmap, h_featmap)
    nh, ne = attentions.shape
    side = int(np.sqrt(ne))
    attentions = attentions.reshape(nh, side, side)

    # NOTE: drop first row and first column
    attentions = attentions[:, 1:, 1:]
    # print("attentions.shape is", attentions.shape) # [1, 119, 119]

    attentions = nn.functional.interpolate(attentions.unsqueeze(0), scale_factor=args.patch_size, mode="nearest")[0].cpu().numpy()
    # print("final attentions.shape is", attentions.shape) # [1, 952, 952]

    # save attentions heatmaps
    os.makedirs(args.output_dir, exist_ok=True)
    torchvision.utils.save_image(torchvision.utils.make_grid(img, normalize=True, scale_each=True), os.path.join(args.output_dir, "img.png"))
    for j in range(nh):
        # NOTE: add exp_id to save to different path
        fname = os.path.join(args.output_dir, f"{args.exp_id}" + '_' "attn-head" + str(j) + ".png")
